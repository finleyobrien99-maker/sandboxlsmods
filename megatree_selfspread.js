// ============================================================================
//  MEGATREE  (Self-spreading "world domination" variant)   for Sandboxels
...
//      through soil — potentially crossing the whole screen — leaving Mega
//      Fibre behind. A small random fraction of settled roots become a
//      persistent MEGA TAPROOT instead of fibre.
//   4. WORLD DOMINATION: once a taproot has aged past maturity (by which point
//      the tree above is fully grown), it VERY rarely sends a Mega Shoot
//      climbing straight up through the ground. When the shoot breaks the
//      surface it sprouts a brand-new Mega Sapling — a whole new tree.
//      Each generation increments `gen`; it stops at `maxGen`, and a packed
//      map naturally halts it (shoots need open air to sprout into).
//
//  Tune everything in the CONFIG block below, then reload the game.
//  Spread too aggressive/laggy? Lower sproutChance / taprootRatio / maxGen.
//  Want faster conquest? Raise them.
// ============================================================================
...
window.MEGATREE_CONFIG = {
    ...
    rootSettleAfter:  8,        // ticks with no soil left nearby -> root settles

    // --- self-spread ---
    taprootRatio:     0.04,     // fraction of settling roots that become a live taproot
    sproutMinAge:     3000,     // a taproot must be this old before it can make a new tree
    sproutChance:     0.0006,   // per-tick chance a *mature* taproot fires a shoot
    maxGen:           6,        // how many generations of self-seeding before it stops
};
...
function mtDiggable(el) {
    return mtIsSoil(el) || el === "mega_root" || el === "mega_fiber" || el === "mega_taproot";
}
...
elements.mega_sapling = {
    ...
    tick: function(pixel) {
        ...
        if (pixel.gen       === undefined) pixel.gen       = 0; // hand-placed trees are gen 0
        ...
        if (!outOfBounds(pixel.x, pixel.y + 1)) {
            ...
            if (below && mtIsSoil(below.element)) {
                ...
                below.still = 0;
                below.gen = pixel.gen; // pass this tree's generation to its roots
            }
        }
...
elements.mega_root = {
    ...
    tick: function(pixel) {
        ...
        if (!soilNear && !ate) {
            pixel.still++;
            if (pixel.still > C.rootSettleAfter) {
                // Most settled roots go inert (fibre). A sparse few stay alive
                // as taproots — these are the seed points for new trees.
                if (Math.random() < C.taprootRatio) {
                    changePixel(pixel, "mega_taproot"); // keeps pixel.gen
                } else {
...
// ============================================================================
//  MEGA TAPROOT  — a sparse, persistent root node. Once it has aged past
//                  maturity (the tree above is long since fully grown), it
//                  VERY rarely fires a shoot straight up, then spends itself.
// ============================================================================
elements.mega_taproot = {
    ...
    tick: function(pixel) {
        ...
        // mature enough + still within the generation budget + rare roll
        if (pixel.age > C.sproutMinAge && pixel.gen < C.maxGen && Math.random() < C.sproutChance) {
            var ax = pixel.x, ay = pixel.y - 1;
            if (!outOfBounds(ax, ay)) {
                var ap = pixelMap[ax][ay];
                if (ap === undefined) {
                    // already at the surface: sprout a tree directly
                    createPixel("mega_sapling", ax, ay);
                    var sp = pixelMap[ax][ay];
                    if (sp) sp.gen = pixel.gen + 1;
                    changePixel(pixel, "mega_fiber"); // spent
                } else if (mtDiggable(ap.element)) {
                    // underground: launch a shoot to climb to the surface
                    changePixel(ap, "mega_shoot");
                    ap.gen = pixel.gen + 1;
                    changePixel(pixel, "mega_fiber"); // spent
                }
                // else: capped by rock/wood above — stay a taproot, try later
            }
        }
        doDefaults(pixel);
    },
...
//                leaving a fibre trail, and pops out as a new Mega Sapling
//                the moment it reaches open air.
// ============================================================================
elements.mega_shoot = {
    name: "Mega Shoot",
    color: ["#8a7860", "#6b5b45"],
    tick: function(pixel) {
        if (pixel.start === pixelTicks) { doDefaults(pixel); return; } // ~1px/frame
        if (pixel.gen === undefined) pixel.gen = 1;

        var ax = pixel.x, ay = pixel.y - 1;

        if (outOfBounds(ax, ay)) {            // reached the top of the world
        ...
        var ap = pixelMap[ax][ay];
        if (ap === undefined) {
            // Broke the surface into open air — sprout a new tree.
            createPixel("mega_sapling", ax, ay);
            var sp = pixelMap[ax][ay];
            if (sp) sp.gen = pixel.gen;
            changePixel(pixel, "mega_fiber");  // leave a trail behind
        } else if (mtDiggable(ap.element)) {
            // Dig upward one pixel, leaving fibre behind.
            changePixel(ap, "mega_shoot");
            ap.gen = pixel.gen;
            changePixel(pixel, "mega_fiber");
        } else {
            // Blocked by rock/wood/etc — give up and go inert.
            changePixel(pixel, "mega_fiber");
        }
...
//  browser console (F12) on an already-running game. If the game has already
//  booted, we finalize our elements and add the menu button ourselves.
//  When loaded normally as a mod (before boot), this does nothing.
// ============================================================================
(function() {
    try {
        var booted = typeof elements === "object" && elements.dirt && elements.dirt.colorObject !== undefined;
        if (!booted) return; // normal mod load: the game will finalize us at boot
        var megas = ["mega_sapling","mega_wood","mega_branch","mega_leaf","mega_root","mega_fiber","mega_taproot","mega_shoot"];
        for (var i = 0; i < megas.length; i++) {
            var k = megas[i];
            if (!elements[k] || elements[k].colorObject !== undefined) continue; // already finalized
        ...
        if (typeof promptText === "function") promptText("Megatree injected! Find 'Mega Sapling' in the Life category.", undefined, "Megatree");
