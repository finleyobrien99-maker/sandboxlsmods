...
window.ULTRATREE_CONFIG = {
    ...
    // --- parasitism ---
    blightSpreadChance: 0.12,   // per-neighbour, per-tick blight crawl through a host
    blightDigestMin:    60,     // ticks before a blighted pixel dissolves into food
    blightDigestMax:    160,
    feedChance:         0.35,   // per-tick chance a taproot converts stored food
    fiberFeedChance:    0.003,  // per-tick chance a fibre pixel does the same
    feedChunk:          40,     // max food (=root length) one new tip is given
};

// Global food pool: every digested host pixel adds 1; taproots spend it on
// new root growth at the same 1:1 rate.
window.ULTRA_FOOD = window.ULTRA_FOOD || 0;
...
var UT_GROUND = ["dirt","mud","sand","wet_sand","packed_sand","gravel","clay",
    "clay_soil","mulch","mudstone","permafrost","snow","packed_snow",
    "tuff","limestone","rock","basalt","color_sand"];
...
// ---- PARASITISM -----------------------------------------------------------
// The ultra tree no longer copies other trees — it EATS them.
// UT_TAKEOVER: reactions carried by the ultra tree's STRUCTURAL parts only
// (sapling, trunk wood, roots, fibre, taproots, shoots). Contact with a
// host's structural matter starts the blight. Leaves and branches — on
// either side — cannot start a takeover; canopies brushing is harmless.
var UT_TAKEOVER = {
    "wood":           { elem2: "ultra_blight", chance: 0.1 },
    "mega_wood":      { elem2: "ultra_blight", chance: 0.1 },
    "sapling":        { elem2: "ultra_blight", chance: 0.1 },
    "mega_sapling":   { elem2: "ultra_blight", chance: 0.1 },
    "root":           { elem2: "ultra_blight", chance: 0.1 },
    "fiber":          { elem2: "ultra_blight", chance: 0.1 },
    "mega_root":      { elem2: "ultra_blight", chance: 0.1 },
    "mega_fiber":     { elem2: "ultra_blight", chance: 0.1 },
    "mega_taproot":   { elem2: "ultra_blight", chance: 0.1 },
    "mega_shoot":     { elem2: "ultra_blight", chance: 0.1 },
    "mycelium":       { elem2: "ultra_blight", chance: 0.08 },
    "mushroom_stalk": { elem2: "ultra_blight", chance: 0.08 },
    "grass":          { elem2: "ultra_blight", chance: 0.06 },
    "vine":           { elem2: "ultra_blight", chance: 0.06 },
    "cactus":         { elem2: "ultra_blight", chance: 0.06 },
    "bamboo":         { elem2: "ultra_blight", chance: 0.06 },
    "seeds":          { elem2: "ultra_blight", chance: 0.04 },
};
// UT_BLIGHTABLE: once inside a host, the blight crawls through ALL of its
// organic matter — canopy included — dissolving every pixel into food.
var UT_BLIGHTABLE = ["wood","tree_branch","plant","sapling","seeds","root","fiber",
    "grass","vine","cactus","pinecone","bamboo","dead_plant","mycelium",
    "mushroom_spore","mushroom_stalk","mushroom_gill","mushroom_cap",
    "mega_sapling","mega_wood","mega_branch","mega_leaf","mega_root",
    "mega_fiber","mega_taproot","mega_shoot"];
function utBlightable(el) { return UT_BLIGHTABLE.indexOf(el) !== -1; }
...
elements.ultra_root = {
    ...
    tick: function(pixel) {
        ...
        for (var k = 0; k < cand.length; k++) {
            ...
            if (!(pixel.grace > 0) && utCrowdCount(nx, ny, pixel) > 1) continue; // tolerant spacing (fed tips burrow first)
            ...
            np.vert = pixel.vert;
            np.life = pixel.life - 1;
            if (pixel.grace > 0) np.grace = pixel.grace - 1;
...
// Shared: spend stored food on a fresh root tip next to (px,py). 1 food = 1px.
function utFeed(pixel) {
    var C = window.ULTRATREE_CONFIG;
    var fdirs = [[-1, 0], [1, 0], [0, 1], [-1, 1], [1, 1]];
    var fd = fdirs[Math.floor(Math.random() * fdirs.length)];
    var fx = pixel.x + fd[0], fy = pixel.y + fd[1];
    if (outOfBounds(fx, fy)) return;
    var fq = pixelMap[fx][fy];
    if (fq && utDiggable(fq.element) && !utRootFamily(fq.element)) {
        var grab = Math.min(window.ULTRA_FOOD, C.feedChunk);
        window.ULTRA_FOOD -= grab;
        changePixel(fq, "ultra_root");
        fq.gen   = pixel.gen || 0;
        fq.dir   = Math.random() < 0.5 ? -1 : 1;
        fq.vert  = Math.random() < 0.4;
        fq.life  = grab;
        fq.grace = 6; // burrow out of the crowded strand zone first
    }
}

// ============================================================================
//  ULTRA FIBRE  — the thick strand body; drinks from the food pool too
// ============================================================================
elements.ultra_fiber = {
    ...
    behavior: behaviors.STURDYPOWDER,
    tick: function(pixel) {
        if (window.ULTRA_FOOD > 0 && Math.random() < window.ULTRATREE_CONFIG.fiberFeedChance) utFeed(pixel);
    },
    reactions: UT_TAKEOVER,
    tempHigh: 500,
    stateHigh: "dirt",
...
// ============================================================================
//  ULTRA BLIGHT  — a host pixel being digested. Crawls through the whole
//                  host organism (canopy included), then each pixel dissolves
//                  into the global food pool that fuels new root growth.
// ============================================================================
elements.ultra_blight = {
    name: "Ultra Blight",
    color: ["#e2d5ee", "#d3bfe6", "#c5a8de", "#efe6f7"],
    tick: function(pixel) {
        var C = window.ULTRATREE_CONFIG;
        if (pixel.start === pixelTicks) { return; }
        if (pixel.digest === undefined) pixel.digest = mtRand(C.blightDigestMin, C.blightDigestMax);

        // Crawl through the host's connected organic matter.
        for (var dx = -1; dx <= 1; dx++) {
            for (var dy = -1; dy <= 1; dy++) {
                if (dx === 0 && dy === 0) continue;
                if (Math.random() >= C.blightSpreadChance) continue;
                var x = pixel.x + dx, y = pixel.y + dy;
                if (outOfBounds(x, y)) continue;
                var q = pixelMap[x][y];
                if (q && utBlightable(q.element)) changePixel(q, "ultra_blight");
            }
        }

        // Digest: dissolve into food.
        pixel.digest--;
        if (pixel.digest <= 0) {
            window.ULTRA_FOOD = (window.ULTRA_FOOD || 0) + 1;
...
elements.ultra_taproot = {
    ...
    tick: function(pixel) {
        ...
        // FEED: convert stored food (digested host matter) into brand-new root
        // growth at 1:1 — a tip of length N costs exactly N food.
        if (window.ULTRA_FOOD > 0 && Math.random() < C.feedChance) utFeed(pixel);
...
// #####################        ROT  FUNGUS        ###########################
// ############################################################################
//  The counter-organism. Place "Rot Fungus" on any tree (vanilla, Mega, or
//  Ultra). It is SLOW and works in two stages:
//    1. WRAP  — it creeps over the tree's air-facing SURFACE only, turning
//       covered pixels into fungus but remembering what they were. It sheathes
//       the whole tree from the outside in.
//    2. DIGEST — once a fungus pixel finds no more exposed surface to cover,
//       it begins dissolving, and digestion cascades TOP-TO-BOTTOM (a pixel
//       only dissolves once the cell above it is already clear).
//  Fungus dissolving in open air puffs into a drifting SPORE CLOUD that rains
//  mushroom spores. Any root touching the fungus rots into mycelium.
// ============================================================================

window.ROT_STATE = { lastSurfaceTick: 0 };
window.ROT_CONFIG = {
    wrapChance:    0.08,   // per-tick chance a fungus pixel sheathes a surface neighbour
    sealTicks:     40,     // ticks with no exposed surface nearby -> switch to digest
    digestChance:  0.06,   // per-tick chance a digest-ready fungus pixel dissolves
    cloudChance:   0.7,    // chance a surface dissolve becomes a spore cloud (vs vanish)
    rootFlowChance: 0.6,   // how fast digest travels down a root network
    sporeDrop:     0.02,   // per-tick chance a spore cloud drops a mushroom spore
    cloudDrift:    0.3,    // per-tick chance a spore cloud drifts sideways
    cloudLife:     600,    // spore cloud lifespan (ticks)
};

// Tree matter the fungus WRAPS (trunk/branch/leaf/sapling of every variant).
var ROT_WRAP = ["wood","tree_branch","plant","sapling","seeds","dead_plant","pinecone",
    "mega_sapling","mega_wood","mega_branch","mega_leaf","mega_shoot",
    "ultra_sapling","ultra_wood","ultra_branch","ultra_leaf","ultra_shoot","ultra_blight"];
function rotWrappable(el) { return ROT_WRAP.indexOf(el) !== -1; }
// Roots of every variant -> these rot into mycelium on contact.
var ROT_ROOTS = ["root","fiber","mega_root","mega_fiber","mega_taproot",
    "ultra_root","ultra_fiber","ultra_taproot"];
// Does (x,y) touch open air?
function rotAtAir(x, y) {
    for (var i = 0; i < 4; i++) {
        var nx = x + [0,0,-1,1][i], ny = y + [-1,1,0,0][i];
...
elements.rot_fungus = {
    name: "Rot Fungus",
    color: ["#8a9a5b", "#6e7d4a", "#9caf6a", "#5f6b40"],
    tick: function(pixel) {
        var C = window.ROT_CONFIG;
        if (pixel.start === pixelTicks) { doDefaults(pixel); return; }
        if (!window.ROT_STATE) window.ROT_STATE = { lastSurfaceTick: pixelTicks };

        // Scan neighbours: exposed tree surface, all adjacent tree, adjacent roots.
        var exposed = [], adjacentTree = [], adjacentRoots = [];
        for (var dx = -1; dx <= 1; dx++) {
            for (var dy = -1; dy <= 1; dy++) {
                if (dx === 0 && dy === 0) continue;
                var x = pixel.x + dx, y = pixel.y + dy;
                ...
                var q = pixelMap[x][y];
                if (!q) continue;
                if (rotWrappable(q.element)) {
                    adjacentTree.push(q);
                    if (rotAtAir(x, y)) exposed.push(q);
                } else if (ROT_ROOTS.indexOf(q.element) !== -1) {
                    adjacentRoots.push(q);
                }
            }
        }

        // While ANY fungus still sees exposed tree surface, the whole
        // infestation stays in WRAP mode — digestion waits for full coverage.
        if (exposed.length > 0) window.ROT_STATE.lastSurfaceTick = pixelTicks;
        var wrapping = (pixelTicks - window.ROT_STATE.lastSurfaceTick <= 2);

        if (wrapping) {
            // Sheathe exposed surface only — creep around the outside.
            for (var e = 0; e < exposed.length; e++) {
                if (Math.random() < C.wrapChance) {
                    var host = exposed[e].element;
                    changePixel(exposed[e], "rot_fungus");
                    exposed[e].host = host;
                }
            }
        } else {
            // DIGEST. Flow through remaining tree matter (trunk core included)...
            for (var a = 0; a < adjacentTree.length; a++) {
                if (Math.random() < C.wrapChance) {
                    var h2 = adjacentTree[a].element;
                    changePixel(adjacentTree[a], "rot_fungus");
                    adjacentTree[a].host = h2;
                }
            }
            // ...and travel DOWN the root network, carrying host=root so it
            // dissolves into mycelium rather than air.
            for (var r = 0; r < adjacentRoots.length; r++) {
                if (Math.random() < C.rootFlowChance) {
                    var h3 = adjacentRoots[r].element;
                    changePixel(adjacentRoots[r], "rot_fungus");
                    adjacentRoots[r].host = h3;
                }
            }
            // Dissolve top-to-bottom: only when the cell above is clear.
            var above = outOfBounds(pixel.x, pixel.y - 1) ? null : pixelMap[pixel.x][pixel.y - 1];
            var aboveClear = !above || (above.element !== "rot_fungus" && !rotWrappable(above.element));
            if (aboveClear && Math.random() < C.digestChance) {
                if (pixel.host && ROT_ROOTS.indexOf(pixel.host) !== -1) {
                    changePixel(pixel, "mycelium");         // rotted root -> mycelium
                } else if (rotAtAir(pixel.x, pixel.y) && Math.random() < C.cloudChance) {
                    changePixel(pixel, "spore_cloud");      // exposed -> spore cloud
                } else {
                    deletePixel(pixel.x, pixel.y);          // interior -> gone
                }
                return;
            }
        }
...
// ============================================================================
//  SPORE CLOUD  — drifts like a cloud, rains mushroom spores, fades out
// ============================================================================
elements.spore_cloud = {
    name: "Spore Cloud",
    color: ["#b8c4a0", "#a8b490", "#c6d0b2"],
    tick: function(pixel) {
        var C = window.ROT_CONFIG;
        if (pixel.life === undefined) pixel.life = C.cloudLife;
        pixel.life--;
        if (pixel.life <= 0) { deletePixel(pixel.x, pixel.y); return; }

        // Drop spores.
        if (Math.random() < C.sporeDrop && isEmpty(pixel.x, pixel.y + 1)) {
            createPixel("mushroom_spore", pixel.x, pixel.y + 1);
        }

        // Buoyant drift: rise slowly, wander sideways.
        if (Math.random() < 0.15 && isEmpty(pixel.x, pixel.y - 1)) {
            movePixel(pixel, pixel.x, pixel.y - 1);
        } else if (Math.random() < C.cloudDrift) {
            var d = Math.random() < 0.5 ? -1 : 1;
            if (isEmpty(pixel.x + d, pixel.y)) movePixel(pixel, pixel.x + d, pixel.y);
        }
        doDefaults(pixel);
    },
    tempHigh: 250,
...
(function() {
    try {
        ...
        if (!booted) return;
        var megas = ["mega_sapling","mega_wood","mega_branch","mega_leaf","mega_root","mega_fiber","mega_taproot","mega_shoot","ultra_sapling","ultra_wood","ultra_branch","ultra_leaf","ultra_root","ultra_fiber","ultra_taproot","ultra_shoot","ultra_blight","rot_fungus","spore_cloud"];
        ...
        if (typeof promptText === "function") promptText("Megatree injected! Mega Sapling, Ultra Sapling and Rot Fungus are in the Life category.", undefined, "Megatree");
